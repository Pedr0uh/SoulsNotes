...
@Composable
fun UndertaleSondsScreen(NavController: NavHostController){

    val context = LocalContext.current //variavel que armazena o contexto da aplicação (obrigatorio para player)

    val musicList = listOf(
        RawResourceDataSource.buildRawResourceUri(R.raw.undertale_tema),
        RawResourceDataSource.buildRawResourceUri(R.raw.waterfall_tema),
        RawResourceDataSource.buildRawResourceUri(R.raw.fallen_down)
    )
    ...
    val exoPlayerMusic = remember {
        ExoPlayer.Builder(context).build().apply {
            setMediaItem(MediaItem.fromUri(musicList[currentTrackIndex]))
            prepare()
            playWhenReady = false
        }
    }
    ...
    val exoPlayer = remember {
        ExoPlayer.Builder(context).build().apply {
            val videoUri = RawResourceDataSource.buildRawResourceUri(R.raw.frisk_waterfall) //armazena o video
    ...
    LaunchedEffect(currentTrackIndex) {
        exoPlayerMusic.setMediaItem(MediaItem.fromUri(musicList[currentTrackIndex]))
    ...
    Column(
        ...
        Text(
           text = "Música ${currentTrackIndex + 1}",
           color = Color.White,
        ...
        Row(
            ...
            IconButton(
                ...
                val iconRes = if (exoPlayerMusic.isPlaying) R.drawable.pause else R.drawable.reproduzir
                Image(
